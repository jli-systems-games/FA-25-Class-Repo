...
public class AnimalData : ScriptableObject
{
    ...
    [Header("🆕 食性配置")]
    [Tooltip("食性类型")]
    public DietType dietType = DietType.Herbivore;

    [Tooltip("目标植物类型（草食/杂食用）")]
    public PlantType requiredPlantType = PlantType.Flowering;

    [Tooltip("目标动物类型（肉食/杂食用）")]
    public AnimalType[] preyAnimalTypes = new AnimalType[0];  // 捕食目标

    [Header("生成条件")]
    [Tooltip("需要的最少植物数量")]
    public int minimumPlantCount = 3;

    [Tooltip("🆕 需要的最少猎物数量（肉食动物用）")]
    public int minimumPreyCount = 2;
