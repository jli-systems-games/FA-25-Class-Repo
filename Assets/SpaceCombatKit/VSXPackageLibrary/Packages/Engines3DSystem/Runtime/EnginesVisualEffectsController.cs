...
namespace VSX.Engines3D
{
    ...
    public class EnginesVisualEffectsController : MonoBehaviour
    {
		...
		protected virtual void Awake()
		{
            ...
            // Cache all the glow materials (创建材质实例副本)
            ...
            // Cache all the halo materials (创建材质实例副本)
            ...
            // Cache all the trail renderer materials (创建材质实例副本)
        ...
        {
            // 清理所有创建的材质实例，防止严重内存泄漏
            foreach (Material mat in exhaustGlowMaterials)
            {
                if (mat != null) Destroy(mat);
            }
            exhaustGlowMaterials.Clear();
            
            foreach (Material mat in exhaustHaloMaterials)
            {
                if (mat != null) Destroy(mat);
            }
            exhaustHaloMaterials.Clear();
            
            foreach (Material mat in exhaustParticleMaterials)
            {
                if (mat != null) Destroy(mat);
            }
            exhaustParticleMaterials.Clear();
            
            foreach (Material mat in distortionParticleMaterials)
            {
                if (mat != null) Destroy(mat);
            }
            distortionParticleMaterials.Clear();
            
            foreach (Material mat in exhaustTrailMaterials)
            {
                if (mat != null) Destroy(mat);
            }
            exhaustTrailMaterials.Clear();
        }
