...
namespace VSX.Health
{
    ...
    public class EnergyShieldControllerBase : MonoBehaviour
    {
        ...
        // 缓存材质实例以避免每帧创建新副本
        protected Material cachedShieldMaterial;
        ...
        protected virtual void Awake()
        {
            ...
            {
                cachedShieldMaterial = energyShieldMeshRenderer.material;
            }
        ...
        {
            // 清理创建的材质实例
            if (cachedShieldMaterial != null)
            {
                Destroy(cachedShieldMaterial);
                cachedShieldMaterial = null;
            }
        }
        ...
        protected virtual void UpdateEffects()
        {
            ...
            if (cachedShieldMaterial == null || energyShieldMeshRenderer == null) return;
            ...
            if (hitAnimatedRimGlow)
            {
                ...
                cachedShieldMaterial.SetFloat("_RimOpacity", amount);
            }
            ...
            for (int i = 0; i < effectInstances.Length; ++i)
            {
                ...
                cachedShieldMaterial.SetVector("_EffectPosition" + i.ToString(), tmp);
            }

            // Update the effect colors in the shader
            cachedShieldMaterial.SetVector("_EffectColor0", effectInstances[0].color);
            cachedShieldMaterial.SetVector("_EffectColor1", effectInstances[1].color);
            cachedShieldMaterial.SetVector("_EffectColor2", effectInstances[2].color);
            cachedShieldMaterial.SetVector("_EffectColor3", effectInstances[3].color);
            cachedShieldMaterial.SetVector("_EffectColor4", effectInstances[4].color);
            cachedShieldMaterial.SetVector("_EffectColor5", effectInstances[5].color);
            cachedShieldMaterial.SetVector("_EffectColor6", effectInstances[6].color);
            cachedShieldMaterial.SetVector("_EffectColor7", effectInstances[7].color);
            cachedShieldMaterial.SetVector("_EffectColor8", effectInstances[8].color);
            cachedShieldMaterial.SetVector("_EffectColor9", effectInstances[9].color);
