...
public partial class FlexibleBlurPass : ScriptableRenderPass
{
    ...
    private static Matrix4x4 OverlayUIViewMatrix = Matrix4x4.identity;
    ...
    private readonly bool uiBlurLayersSeeLower, blurredImageSeeUIBlurs, blurredImageLayersSeeLower, useComputeShaders, overlayCompatibilityFix;
    ...
    private void SharedSetup(int cameraPixelHeight, float renderScale)
    {
        ...
        var (width, height) = (Screen.width * 0.5f, Screen.height * 0.5f);
        OverlayUIViewMatrix = Matrix4x4.Translate(new Vector3(-width, -height, 0f));
        OverlayUIProjectionMatrix = Matrix4x4.Ortho
        (
            -width,
            width,
            -height,
    ...
    public void Setup(ScriptableRenderer _, in RenderingData renderingData)
    {
        ...
        SharedSetup(camData.camera.pixelHeight, camData.renderScale);
            ...
            void FinalBlitToDestination()
            {
                ...
                if (useQuadBlit)
                {
                    ...
                    else
                    {
                        ...
                        cmd.SetViewMatrix(uiCamera?.worldToCameraMatrix ?? OverlayUIViewMatrix);
                        BlitToQuad(cmd, Temp1Id, destination, quadBlitsMat, transformationMatrix, blurRegion, blurRegionRight, originalWidth, originalHeight, 0);
