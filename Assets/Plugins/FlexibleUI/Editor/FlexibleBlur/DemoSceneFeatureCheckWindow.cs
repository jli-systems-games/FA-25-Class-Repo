...
public class DemoSceneFeatureCheckWindow : EditorWindow
{
    ...
    static void StartupCheck(Scene scene, OpenSceneMode _)
    {
        ...
        if (!scene.path.ToLower().Contains("flexibleui"))
            return;
        ...
        if (defaultPipelineAsset.rendererDataList.ToArray().Any(x => x.TryGetRendererFeature<FlexibleBlurFeature>(out var _)))
            return;
#else
...
#if UNITY_2023_2_OR_NEWER
        window.rendererData = defaultPipelineAsset.rendererDataList[defaultRendererIdx];
#else
