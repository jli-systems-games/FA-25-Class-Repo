...
public class CharacterController2D : MonoBehaviour
{
	...
	const float k_GroundedRadius = .3f; // Radius of the overlap circle to determine if grounded (增加检测范围)
    ...
    private void Start()
    {
        // 从 GameManager 同步能力状态
        if (GameManager.Instance != null)
        {
            // 同步能力状态。注意这里不应该是"强制开启"，而是"同步"。
            // 如果 GameManager 说解锁了，那就设为 true。
            // 如果 GameManager 没解锁，那就应该保持为 false (或者你 Inspector 里设置的默认值)。
            // 但为了确保逻辑一致性，这里应该完全依赖 GameManager 的状态。
            enableDoubleJump = GameManager.Instance.hasUnlockedDoubleJump;
            enableDash = GameManager.Instance.hasUnlockedDash;
            enableWallSlide = GameManager.Instance.hasUnlockedWallSlide;
        }
    }
	...
	private void FixedUpdate()
	{
		...
		// 调试：显示检测到的碰撞体数量
		if (colliders.Length > 0 && !m_Grounded)
		{
			Debug.Log("检测到 " + colliders.Length + " 个地面碰撞体");
		}
		
		for (int i = 0; i < colliders.Length; i++)
		{
			if (colliders[i].gameObject != gameObject && !colliders[i].CompareTag("Interactable"))
			{
				m_Grounded = true;
				if (!wasGrounded )
				{
	...
	{
		if (canMove) {
			if (dash && canDash && !isWallSliding && enableDash)
			{
			...
			else if (!m_Grounded && jump && canDoubleJump && !isWallSliding && enableDoubleJump)
			{
				canDoubleJump = false;
			...
			else if (m_IsWall && !m_Grounded && enableWallSlide)
			{
				...
				else if (dash && canDash && enableDash)
				{
					isWallSliding = false;
					animator.SetBool("IsWallSliding", false);
					oldWallSlidding = false;
